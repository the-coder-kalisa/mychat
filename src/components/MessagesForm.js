...
function MessagesForm() {
  ...
  var yesterday = day-1;
  yesterday = yesterday.length > 1 ? yesterday : '0' + yesterday;
  const yesterdayDate = yesterday + '/' + month + '/' + year;
  ...
  return (
    <div className="w-[80vh] border-x-2 rounded-lg shadow-md border-t-2 border-solid border-[#c8c8c8] h-full relative">
      {messages.length > 0 ? <div className="overflow-auto relative h-[63vh]">
        ...
        return(
        <div key={index}>
          <p className="bg-[#1a9797] rounded-lg text-white shadow-md drop-shadow-md shadow-[#000000b3] sticky z-40 w-[100%] overflow-hidden top-0 h-12 justify-center flex items-center"><span className="text-lg font-semibold">{_id === todayDate ? 'Today' : `${_id === yesterdayDate ? 'Yesterday' : _id}`}</span></p>
          <div className="z-10">
          {messagesByDate?.map(({content, from: sender, time}, index)=>{
           return(
             <div key={index} className={`${sender._id === user._id ? 'justify-end' : 'justify-start'} flex mb-3`}>
               <div className={`ml-[20px] message-inner p-[10px] min-w-[200px] max-w-[90%] text-left min-h-[80px] font-[sans-serif] font-[500] text-[1rem] inline-block rounded-[10px] ${sender._id === user._id ? 'bg-[#ffdab9]' : 'bg-[#d1e7dd]'}`}>
